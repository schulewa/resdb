...
export class EventTypeGroupService {
  ...
  findAll(): Observable<EventTypeGroup[]> {
    ...
    return this.http.get<EventTypeGroup[]>(url).pipe(catchError(this.handleError));
  }
  ...
  delete(entityType: EventTypeGroup): Observable<EventTypeGroup> {
    const url = this.constructUrlWithId(ResdbUrlEndpoints.EVENT_TYPE_GROUP_URL, entityType.id);
