...
public enum ConfigurationPropertyName {
    ...
    public static ConfigurationPropertyName getForName(@NotEmpty String name) {
      return Arrays.stream(ConfigurationPropertyName.values())
        .filter(e -> name.equals(e.getName()))
        .findFirst()
        .orElseThrow(() -> new IllegalArgumentException("No ConfigurationPropertyName found for name [" + name + "]"));
    }
