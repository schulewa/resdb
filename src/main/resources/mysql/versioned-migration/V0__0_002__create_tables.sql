...
(
  ...
  permission                INTEGER     	NOT NULL,
	CONSTRAINT resdb_menu_item_pk PRIMARY KEY (menu_id, level, sequence_no)
);
CREATE UNIQUE INDEX resdb_menu_item_idx1 ON resdb_menu_item (menu_id, internal_menu_id, item_text);
CREATE UNIQUE INDEX resdb_menu_item_idx2 ON resdb_menu_item (menu_id, level, sequence_no);
...
CREATE UNIQUE INDEX resdb_reference_name_idx ON resdb_reference (reference_type_id, publication_id, relates_to_theory_id, relates_to_tale_id);
...
CREATE UNIQUE INDEX resdb_user_password_idx ON resdb_user_password (logon_name, valid_to);
